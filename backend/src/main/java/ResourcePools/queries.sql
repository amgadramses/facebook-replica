...
CREATE OR REPLACE FUNCTION public.show_profile(user_id integer) RETURNS refcursor AS $$ DECLARE cursor REFCURSOR := 'cur'; BEGIN OPEN cursor FOR (SELECT *, 1 AS type FROM Users U INNER JOIN Education E ON E.user_id = U.user_id WHERE U.user_id = $1) UNION (SELECT *, 2 AS type FROM Users U INNER JOIN work W ON W.user_id = U.user_id WHERE U.user_id = $1); RETURN cursor; END; $$ LANGUAGE PLPGSQL;
